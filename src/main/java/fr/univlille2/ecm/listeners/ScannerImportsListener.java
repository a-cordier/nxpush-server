...
public class ScannerImportsListener implements EventListener {
	...
	public void handleEvent(Event event) throws ClientException {
		...
		DocumentModel sourceDocument = ((DocumentEventContext) context)
				.getSourceDocument();
			...
			UnrestrictedSessionRunner runner = new UnrestrictedSessionRunner(
				...
				public void run() throws ClientException {
					try { /* move document and remove name prefix */
						String name = routedDocument.getName().replaceFirst(
								"[0-9]{4}[A-Za-z]{2}[-_]*", "");
						if (isUntitled(name)) {
							name = UNTITLED_DOC_NAME
									+ (name.lastIndexOf('.') >= 0 ? name
											.substring(name.lastIndexOf('.'))
											: "");
						}
						DocumentModel targetDocument = session.move(
								routedDocument.getRef(), destination, name);
